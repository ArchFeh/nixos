...
static void sort(const int rank, const int size, int *nums, const int n)
{
	...
	qsort(nums, n, sizeof(int), cmp); //Have sorted array on each process
	...
	while (not_sorted) {
		if ((rank % 2 == 1) && (rank != size - 1)) {
			...
			if ((rank != MASTER) && (rank != size - 1)) {
				MPI_Recv(recv_tmp, n, MPI_INTEGER, rank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
int main(int argc, char **argv)
{
	...
	MPI_Finalize();
	time2 = clock();
	printf("Time:%f ", (time2 - time1) / CLOCKS_PER_SEC);
	return 0;
}